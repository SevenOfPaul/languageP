...
import __wbg_get_imports from "../hooks/generatorWbg.js"
...
export const Wasm=() => {
 async function upP(){
  ...
  useEffect( ()=>{
    (async()=>{
      window.append_to_output = (text, newline) => {
        setRes(prev => prev + text + (newline ? '\n' : ''));
      };
     setWasmModule(await WebAssembly.instantiateStreaming(await fetch("/wasm/p_bg.wasm"),{
      ...__wbg_get_imports()
     }));
    })()
 },[]);
  ...
  function runCode(){
    wasmModule.instance.exports.run_program(val);
  }
    const footer=[
